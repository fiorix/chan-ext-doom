...
`test/test_abi.c` links two objects that reach the headers in opposite orders and compares widths and offsets across `player_t`, `wbplayerstruct_t`, `wbstartstruct_t`, `vertex_t`, `anim_t`, `ceiling_t`, `floormove_t`, `deh_mapping_entry_t` and `net_full_ticcmd_t`. Those are the shared structs whose layout can actually move. `plat_t` and the ST/HU widget structs carry single `boolean` value fields, which measure identically under a split because the difference is absorbed into padding, so equality checks on them do not bite; the assertion that `sizeof(boolean)` is 4 is what really guards them. `animdef_t` is not in scope at all: it lives inside `p_spec.c` rather than a header, it is `PACKED_STRUCT`, and its `istexture` is a `signed char`. The `boolean istexture` in `p_spec.h` belongs to `anim_t`, which is covered.

Two of those diverge measurably under a split `boolean`, and the pattern explains the test's shape: the damage comes from `boolean` *arrays*, where the width multiplies. `player_t` (which holds `cards[6]`) differs 304 against 328 bytes with `killcount` at offset 196 against 216, and `net_full_ticcmd_t` (which holds `playeringame[8]`) differs 176 against 200. A struct with a single `boolean` field mostly absorbs the difference into padding and measures the same either way, which is why per-field reasoning is not a substitute for pinning the width. `net_full_ticcmd_t` is the one with wire consequences: it crosses `net_client`, `net_server` and `d_loop`, so a split there shifts the whole GAMEDATA fan-out. The formats themselves are safe regardless, because `p_saveg` and `net_structrw` read and write explicit widths, so no demo, savegame or packet ever carries the raw ABI.
...
**`src/doom/d_main.c`, `G_BeginRecording` given a call site, and not upstream's.** Upstream begins recording in `D_DoomLoop` immediately before the loop starts, and that site does not work here: this fork registers its emscripten main loop at the top of `D_DoomMain`, so tics can run before `D_DoomLoop` is reached. With `demo_p` still null at that point, `G_WriteDemoTiccmd`'s re-read walks uninitialised memory, finds a stray `0x80` DEMOMARKER and calls `G_CheckDemoStatus`, which clears `demorecording` again with no diagnostic. `G_WriteDemoTiccmd` now fails closed on those invariants, so that path is gone regardless of ordering.
...
A single-player run reports `version=109 skill=2 episode=1 map=1 deathmatch=0`, and two peers report identical headers differing only in `consoleplayer`. A header showing `skill=0 episode=0` with a nonsense `deathmatch` is the signature of the `boolean` ABI split described above rather than a demo fault.

**`-verifydemo` unbinds the demo-quit key for verification runs.** `'q'` is the inherited upstream default for `key_demo_quit` in both lineages, but this fork rebound `key_fire` from right-control to `'q'` for browser friendliness, and `G_WriteDemoTiccmd` polls the quit key on every recorded tic, so together they end a recording on the player's first shot. The default is left at upstream's value and the loader passes `-verifydemo` alongside `-record`, which unbinds the key for that run only. This also stops a persistent configured binding from ending a canary, and leaves ordinary demo recording behaving exactly as upstream does.
...
The format has a known blind spot: weapon psprite animation is excluded, so a divergence living only in mid-fire animation, never reaching `readyweapon`, `pendingweapon`, ammo or `refire`, is not caught. Closing it means adding per-player psprite state index and tics, which is a format change rather than a tuning knob.

The special-thinker set is the `p_saveg` one, and two live thinkers sit outside it. `T_MoveElevator` and `T_FireFlicker` run as ordinary thinkers but are not among the seven classes the serializer writes, so their *phase* is not in the stream. Their effects are: an elevator drives `T_MovePlane`, whose floor and ceiling heights are serialized, and a flicker writes `sector->lightlevel`, which is serialized too. A flicker additionally draws from `P_Random`, so a phase difference perturbs `prndindex`, which the header carries. The gap is therefore narrow rather than open: a divergence is missed only while it remains confined to the internal phase and has not yet produced a different height, light level or RNG draw. Closing it fully means writing elevator direction and destination and the flicker counters, which is a format change of the same class as the psprite gap above.

## Automatic verdict

Both canary lines are text on a console nobody reads during a two-window run, so the loader pages compare them and display an aggregate verdict: match, mismatch, or pending. The reducer is `canary_verdict.js`, shared verbatim between the pages and the tests, so the logic under test is the logic that ships.
...
**Trust boundary.** Pages pair over a `BroadcastChannel`, which every page on the origin can read and write. Session ids are therefore public to any same-origin participant. A malicious same-origin page can read the live partner's session id, name it in `replaces`, displace the honest peer, reflect a later report back, and leave the victim displaying a stable match while the real peer stays locked out. The protocol authenticates nobody. A shared token would not change this, because it would be readable through the same origin; and code that can do any of the above can equally patch the reducer or write the verdict into the DOM directly. So state it plainly: the verdict is evidence only while every script and page served from that origin is trusted. It is a desync detector for a cooperating pair, not a security control.

The perimeter is therefore not just the origin's served files but **every live tab and window on that origin**, because any one of them can reach the channel. A page that is merely open counts. Nothing in the code enforces that, so it is enforced operationally: close every other tab and window on the origin, and run with exactly the two prepared windows, both visible.

The perimeter grows with the origin. Serving this verdict from a larger application origin puts every script running there inside the boundary, which is a far wider surface than two purpose-opened windows. The verdict is a lobby convenience and a desync aid; it is not a security signal, and it cannot become one by being embedded somewhere busier.

**Recovery and what a result is worth.** The in-page Relaunch button carries session replacement correctly and is the supported way to restart. A whole-page F5 is different, and it does not reset both sides: the reloaded page loses its identity and shows pending, while the *surviving* page keeps its old match against a session that no longer exists, indefinitely. Recovery is reloading or relaunching both windows. Adopting a partner on a timeout would paper over this and reopen third-page takeover, so it is deliberately absent.

A verdict is therefore only evidence when **both live windows show a match simultaneously after the same shared exit**. One window's retained match proves nothing on its own, because it may be describing a peer that is already gone.
...
The fingerprint is JSON rather than delimiter-joined fields. A filename may legitimately contain `:` and `|`, so concatenation is ambiguous: a single entry whose name embeds the separators produces exactly the string two ordinary entries produce. The regression test carries that exact collision case.
...
Configure and a full link complete with emsdk 6.0.3, and both routes are deterministic: two fresh clean-archive builds of each produce identical bytes.

Both routes pin `-std=gnu99`, which is the precondition for comparing them at all. Compiling the two routes in different language modes makes them differ by 83 wasm bytes for that reason alone, which is easy to mistake for a linker artifact. With the language mode equal on both sides:
...
- The data sections carry the whole +3, but the difference is far broader than that number suggests. Both declare 2160 segments, and the segments are split at different boundaries: the first difference is a header whose `i32.const` offset differs by four and whose declared length differs. The reordering runs through the whole section rather than being a single shifted split. The largest end-aligned identical tail is 149 bytes out of 286060; only 10 of the 2160 trailing segments match on both offset and contents; and reconstructing the static memory image from the segments shows it differing across well over a thousand separate address ranges. So the static data is broadly reordered, and the small size delta measures only the packing overhead, not the amount that moved.

CMake compiles each translation unit to its own object and `wasm-ld` merges them, while the recipe hands every source to one `emcc` invocation; a different segment split and a different function order are the expected consequence of that. None of this establishes that the two modules are semantically equivalent. It establishes that they are built from the same sources in the same language mode and differ in static layout; equivalence would need a behavioural comparison, which is not what these numbers are. The pinned hashes are the recipe's, and the recipe is the reference.
...
Every guard here is mutation-checked: removing the behaviour it protects makes the suite fail. The tests are therefore known to be capable of failing rather than merely green.
