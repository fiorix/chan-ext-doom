...
`test/test_abi.c` links two objects that reach the headers in opposite orders and compares widths and offsets across `player_t`, `wbplayerstruct_t`, `wbstartstruct_t`, `vertex_t`, `anim_t`, `ceiling_t`, `floormove_t`, `deh_mapping_entry_t` and `net_full_ticcmd_t`. That is not yet the complete set: `animdef_t`, `plat_t` and the ST/HU widget structs also carry `boolean` value fields and are not covered.
