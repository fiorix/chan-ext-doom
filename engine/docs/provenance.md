...
This was core simulation memory corruption. It surfaced as a demo header recording the wrong episode, which is worth remembering: the visible symptom was several layers away from the fault, and the header only exposed it because the armed line names its fields. `test/test_abi.c` links two objects that reach the headers in opposite orders and compares `sizeof(boolean)`, `sizeof(player_t)` and member offsets.
